...
export default class Stack extends GameModule {
  constructor(parent, ctx) {
	...
	this.mirrorAnimationLimit = 500 - effectDelay
	this.flipAnimation = 1000
	this.flipAnimationLimit = 500 - effectDelay
  ...
  add(passedX, passedY, shape, color) {
	...
	if (this.parent.useEffectBlocks) {
		...
		if (this.effectBlockInterval <= 3 && this.effectBlockInterval > 0) {
			this.parent.stat.effect = locale.getString("ui", "incomingEffect")
		} else {
			this.parent.stat.effect = ""
		}
	}
  ...
  draw() {
    ...
    */
    ...
    if (this.toCollapse.length > 0 && this.parent.useBoneBlocks !== true) {
