...
export default class Game {
  ...
  resize() {
    ...
    for (const element of [
	  ...
	  let cellSize1 = Math.floor(((base / 1.2 / game.settings.height) * (game.userSettings.size * 0.8)) / 100)
	  let cellSize2 = Math.floor(((base / 1.2 / game.settings.height) * (game.userSettings.size * 0.8)) / 100)
	  if (game.settings.height <= 10 && game.settings.width <= 5) {
	    cellSize2 = Math.floor(((base / 1.2 / (game.settings.height * 2)) * (game.userSettings.size * 0.8)) / 100)
	  } else {
	    cellSize2 = Math.floor(((base / 1.2 / game.settings.height) * (game.userSettings.size * 0.8)) / 100)
	  }
  ...
  get cellSize() {
	...
	let resultingCellSize = Math.floor(((base / 1.2 / this.settings.height) * (this.userSettings.size * 0.8)) / 100)
	if (this.settings.height <= 10 && this.settings.width <= 5) {
	resultingCellSize = Math.floor(((base / 1.2 / (this.settings.height * 2)) * (this.userSettings.size * 0.8)) / 100)
	} else {
	resultingCellSize = Math.floor(((base / 1.2 / this.settings.height) * (this.userSettings.size * 0.8)) / 100)
	}
